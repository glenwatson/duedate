...
var duedateApp=angular.module("duedateApp",[]),init=function(){window.checkAuth(!0)};duedateApp.controller("tasklistCtrl",["$scope","$window",function(t,s){function e(a,i){var n={pageToken:i,tasklist:a.id,showDeleted:!0};s.gapi.client.tasks.tasks.list(n).then(function(s){"items"in s.result&&s.result.items.forEach(function(s){t.tasks.push(s)}),"nextPageToken"in s.result&&e(a,s.result.nextPageToken)})}function a(i){var n={pageToken:i};s.gapi.client.tasks.tasklists.list(n).then(function(s){s.result.items.forEach(function(s){t.tasklists.push(s),e(s,null)}),"nextPageToken"in s.result&&a(s.result.nextPageToken)})}var i="954795491695-9pop5kva3tg9ontq87j2lg7oc0fgrv69.apps.googleusercontent.com",n="https://www.googleapis.com/auth/tasks";t.authenticated=void 0,t.checkAuth=s.checkAuth=function(t){s.gapi.auth.authorize({client_id:i,scope:n,immediate:t},l)};var l=function(e){e&&!e.error?(t.authenticated=!0,s.gapi.client.load("tasks","v1",k)):t.authenticated=!1};setInterval(function(){t.$apply()},500),t.tasklists=[],t.tasks=[],t.defaultTasklistID=null;var k=function(){s.gapi.client.tasks.tasklists.get({tasklist:"@default"}).then(function(s){t.defaultTasklistID=s.result.id,a(null)})};t.taskTasklist=function(s){for(var e=/\/lists\/(.*)\/tasks\//.exec(s.selfLink)[1],a=0;a<t.tasklists.length;a++)if(t.tasklists[a].id===e)return t.tasklists[a]},t.isTaskInTasklist=function(s){return function(e){return!e.deleted&&"all"===s||(!e.deleted&&s.id===t.taskTasklist(e).id||!(!e.deleted||"trash"!==s))}},t.tasklistsInsert=function(e){if(t.data.tasklistInput="",e){var a={title:e};s.gapi.client.tasks.tasklists.insert(a).then(function(s){t.tasklists.push(s.result)})}},t.tasklistsDelete=function(e){var a={tasklist:e.id};s.gapi.client.tasks.tasklists["delete"](a).then(function(s){for(var a=0;a<t.tasks.length;a++)e===t.taskTasklist(t.tasks[a])&&(t.tasks.splice(a,1),a--);t.data.selectedTasklist="all",t.tasklists.splice(t.tasklists.indexOf(e),1)})},t.tasklistsPatchTitle=function(t,e){var a={tasklist:t.id,title:e};s.gapi.client.tasks.tasklists.patch(a).then(function(s){t.title=s.result.title})},t.tasksInsert=function(e,a){if(t.data.taskInput="",a){var i={title:a};e.id?i.tasklist=e.id:i.tasklist="@default",s.gapi.client.tasks.tasks.insert(i).then(function(s){t.tasks.push(s.result)})}},t.tasksDelete=function(e){var a={tasklist:t.taskTasklist(e).id,task:e.id};s.gapi.client.tasks.tasks["delete"](a).then(function(t){e.deleted=!0})},t.tasksUnDelete=function(e){var a={tasklist:t.taskTasklist(e).id,task:e.id,deleted:!1};s.gapi.client.tasks.tasks.patch(a).then(function(t){e.deleted=!1})},t.tasksPatchStatus=function(e){var a={tasklist:t.taskTasklist(e).id,task:e.id};"completed"===e.status?(a.completed=(new Date).toISOString(),a.status="completed"):(a.completed=null,a.status="needsAction"),s.gapi.client.tasks.tasks.patch(a).then(function(t){e.completed=t.result.completed,e.status=t.result.status})},t.tasksPatch=function(e,a,i,n,l){var k={tasklist:t.taskTasklist(e).id,task:e.id,title:a,notes:n||null};if(l){var u=new Date(Date.parse(l));k.due=new Date(Date.UTC(u.getUTCFullYear(),u.getUTCMonth(),u.getUTCDate())).toISOString()}else k.due=null;s.gapi.client.tasks.tasks.patch(k).then(function(s){e.title=s.result.title,e.notes=s.result.notes,e.due=s.result.due,i!==t.taskTasklist(e)&&t.tasksModifyTasklist(e,i)})},t.tasksModifyTasklist=function(e,a){var i;i={tasklist:t.taskTasklist(e).id,task:e.id},s.gapi.client.tasks.tasks["delete"](i).then(function(s){t.tasks.splice(t.tasks.indexOf(e),1)}),i=$.extend({tasklist:a.id},e),i.id=void 0,i.selfLink=void 0,s.gapi.client.tasks.tasks.insert(i).then(function(s){t.tasks.push(s.result)})}}]),duedateApp.filter("fromNow",function(){return function(t){var s=moment().subtract(1,"day"),e=moment(),a=moment().add(1,"day"),i=moment(moment.utc(t).format("YYYY-MM-DD"));return i.isSame(s,"day")?"yesterday":i.isSame(e,"day")?"today":i.isSame(a,"day")?"tomorrow":i.fromNow()}});